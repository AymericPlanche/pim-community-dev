    ...
    private function fetchChildrenCategories(
        ...
        $sql = <<<SQL
    		...
    		LEFT JOIN (
        		...
        		WHERE child.parent_id = :parent_category_id_1
        		GROUP BY child.id
        	) as category_with_children on category_with_children.id = c.id
        	WHERE c.parent_id = :parent_category_id_2
SQL;
    ...
    /**
     * @param int $categoriesWithoutCount
     *
     * [
     *     [
     *         'child_id' => 1,
    ...
    private function fetchCategoriesBetween(int $fromCategoryId, int $toCategoryId)
    {
        $sql = <<<SQL
            ...
            WHERE parent.id = :category_to_expand and subchild.id = :category_to_filter_with
